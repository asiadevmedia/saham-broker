...
class Abroker(telepot.Bot):
					...
					if charts in chartscode:
						if time in optiontime:
							if (time == 'H') or (time == 'h'):
								time = '60';
								overtime = '60 Menit';
							elif (time == 'D') or (time == 'd'):
								time = '1440';
								overtime = '1 Hari';
							elif (time == 'W') or (time == 'w'):
								time = '10080';
								overtime = '1 Minggu';
	...
	def dayli(self, tab):
		...
		ABDocs = AB.Documents.Item(0);
		AW = ABDocs.ActiveWindow;
		AW.SelectedTab = tab;
		AW.ExportImage(filename, 720, 480);
		return

	def chart(self, charts, tab):
		pythoncom.CoInitialize();
		AB = client.Dispatch("Broker.Application");
